...
function oddPairFirstLast(num) {
    ...
    if(num === 0){
      ...
      return Number(numStr[0] + 1) + oddPairFirstLast (Number(numStr.slice(1)));
    } else {
      if(Number(numStr[0] + numStr[numStr.length-1]) % 2 === 1){
        return Number(numStr[0] + numStr[numStr.length-1]) + oddPairFirstLast (Number(numStr.substring(1,numStr.length-1)));
      } else {
        return oddPairFirstLast (Number(numStr.substring(1,numStr.length-1)));
      }
    }
}
