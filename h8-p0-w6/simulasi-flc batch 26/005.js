...
function classReport(students) {
  ...
  for (var i = 0; i < students.length; i++) {
    if (objResult[students[i].class] === undefined) {
      objResult[students[i].class] = new Object();
      objResult[students[i].class].pass = [];
      objResult[students[i].class].fail = [];
    }

    var objStudent = new Object();
    objStudent.name = students[i].name;
    objStudent.score = students[i].score;
    if (students[i].score >= 80) {
      objResult[students[i].class].pass.push(objStudent);
    } else {
      objResult[students[i].class].fail.push(objStudent);
    }
  }
  return objResult;
}
