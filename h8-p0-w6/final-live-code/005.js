...
function vendingMoonbucks(pesanan, uang) {
  ...
  objResult.beverages = [];
  objResult.change = uang;
  objResult.keterangan = '';
  for(var i = 0; i < pesanan.length; i++){
    if(menu.hasOwnProperty(pesanan[i]) && objResult.change >= menu[pesanan[i]]){
      objResult.beverages.push(pesanan[i]);
      objResult.change -= menu[pesanan[i]];
    } else {
      counterMiss++;
    }
  }

  if(counterMiss === pesanan.length){
    objResult.keterangan = 'beverages tidak tersedia pada vendingMoonbucks';
  } else if(objResult.change > uangKembalianTersedia){
    objResult.beverages = [];
    objResult.change = uang;
    objResult.keterangan = 'uang kembalian tidak cukup';
  } else if(counterMiss > 0){
    objResult.keterangan = 'ada pesanan yang tidak terbeli';
  }
  return objResult;
}
